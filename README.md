...
- five-step merge ladder **T1 → T2 → T3 → T4 → T5**; reaching T5 now requires 16 same-family T1 copies instead of the old four-copy T3 cap;
- clearly separated tier presentation with progressively stronger frames, scale, markers and Roman badges; T4/T5 are prestige states that reuse the authored T3 silhouette but add unmistakable aura/frame treatment;
...
- deadlock-safe smart Recruit planning that favors useful multi-tier merge cascades, downweights already-maxed T5 lineages and protects the final free slot instead of allowing cross-family emergency fusion;
- T5 same-family consolidation, with compatible mutation twins ascending and capped/incompatible mutation pairs retaining the stronger mutation;
- **36 original authored creature forms across 12 families** with progressive Recruit unlocks through Chapter 24; prestige T4/T5 deliberately extend gameplay without pretending there are 24 additional authored silhouettes;
...
- versioned persistent saves with migrations through **v14**, including automatic 12-slot -> 15-slot board expansion and T4/T5 persistence;
...
The normal chain is `T1 + T1 → T2`, `T2 + T2 → T3`, `T3 + T3 → T4`, `T4 + T4 → T5`. Two T5 units of the same family remain mergeable as the mutation-ascension / board-space safety valve.
...
Tap **Recruit** to buy a new creature. Recruit RNG favors families where a new T1 advances an existing merge cascade, reduces the chance of spawning a lineage that is already represented only by max-tier units, and still guarantees a safe same-family pair when the final slot would otherwise deadlock.
