...
import { createGameSave, type GameSaveV1 } from '../state/save';
import {
  addUnit,
  createStarterBoard,
  firstEmptySlot,
...
export class GameScene extends Phaser.Scene {
  ...
  private baseHp = 100;
  private bossRound = 1;
  private bossHpMax = 520;
  private bossHp = 520;
  private bossAttackClock = 0;
  ...
  public create(): void {
    ...
    const initialSave = this.registry.get('initialSave') as GameSaveV1 | null;
  ...
  public override update(_time: number, delta: number): void {
    if (!this.bossAlive || this.resolvingBoard) return;
  ...
  private updateBossAttack(delta: number): void {
    this.bossAttackClock += delta;
    const interval = Math.max(2100, 3900 - this.bossRound * 90);
    if (this.bossAttackClock < interval) return;
    this.bossAttackClock = 0;
    this.bossView.telegraph(() => {
      if (!this.bossAlive) return;
      const damage = Math.min(22, 7 + this.bossRound * 2);
      this.baseHp = Math.max(0, this.baseHp - damage);
      this.syncUi();
      if (this.baseHp > 0) this.persistSoon();
      this.cameras.main.shake(145, 0.0042);
      this.fx.flashScreen(0xff4f72, 0.16, 180);
      this.fx.burst(540, 1050, 0xff6d85, 12, 220);
      if (this.baseHp <= 0) this.loseRound();
    });
  }
  ...
  private recruitUnit(): void {
    if (this.resolvingBoard || !this.bossAlive) return;
  ...
  private fireUnitAttack(slot: number, unit: BoardUnit): void {
    ...
    if (!origin) return;
    const creature = getCreature(unit.family, unit.level);
    const target = this.bossView.targetPoint();
    ...
    this.tweens.add({
      ...
      onComplete: () => {
        projectile.destroy();
        trail.destroy();
        this.damageBoss(creature.damage, creature.projectileColor);
      }
    });
  }

  private damageBoss(amount: number, color: number): void {
    if (!this.bossAlive) return;
    this.bossHp = Math.max(0, this.bossHp - amount);
    this.bossView.setHealth(this.bossHp, this.bossHpMax);
    this.fx.floatingDamage(540 + Phaser.Math.Between(-120, 120), 470 + Phaser.Math.Between(-45, 75), amount, color);
    this.bossView.hit(color);
  ...
  private defeatBoss(): void {
    ...
    this.resolvingBoard = true;
    this.bossAttackClock = 0;
    const reward = 85 + this.bossRound * 25;
    this.coins += reward;
    this.syncUi();
    this.cameras.main.shake(260, 0.008);
    this.fx.flashScreen(0x8ffcff, 0.2, 260);
    this.fx.burst(540, 565, 0x9cfbff, 30, 310);
    this.fx.burst(540, 565, 0xffdd6b, 24, 370);

    this.bossView.defeat(reward, () => {
      this.bossRound += 1;
      this.bossHpMax = Math.round(520 * Math.pow(1.22, this.bossRound - 1));
      this.bossHp = this.bossHpMax;
      this.baseHp = Math.min(100, this.baseHp + 20);
      this.bossView.reset();
      this.bossAlive = true;
      this.resolvingBoard = false;
      this.syncUi();
      this.fx.showHint(`BOSS ${this.bossRound} INCOMING`, 1015, '#c7f7ff');
      this.persistNow();
    });
  }
  ...
  private loseRound(): void {
    ...
    this.bossAlive = false;
    this.resolvingBoard = true;
    const banner = this.add.text(540, 920, 'FORTRESS FROZEN!', {
  ...
  private restoreSave(save: GameSaveV1): void {
    this.board = save.board;
    this.coins = save.coins;
    this.baseHp = save.baseHp;
    this.bossRound = save.bossRound;
    this.bossHpMax = save.bossHpMax;
    this.bossHp = Math.min(save.bossHp, save.bossHpMax);
  ...
  private syncUi(): void {
    this.hud.update(this.coins, this.baseHp, this.bossRound);
    this.bossView.setHealth(this.bossHp, this.bossHpMax);
  }
}
