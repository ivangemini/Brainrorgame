...
describe('platform selection', () => {
  ...
  it('detects Poki and Playgama from host or embedding referrer', () => {
    ...
    expect(shouldUsePlaygama('example.com', '', 'https://example.org/page')).toBe(false);
  });
  ...
  it('keeps Yandex SDK detection ahead of portal hints', () => {
    ...
    const adapter = createPlatformAdapter({
      ...
      search: '?platform=playgama',
      referrer: 'https://playgama.com/game/test'
    });
    expect(adapter.id).toBe('yandex');
  });

  it('selects CrazyGames, Poki, Playgama and Web through the same factory', () => {
    ...
    expect(createPlatformAdapter({ globalScope: {}, hostname: 'cdn.example.com', search: '', referrer: 'https://playgama.com/game/test' }).id).toBe('playgama');
