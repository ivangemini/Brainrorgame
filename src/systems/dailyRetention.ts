...
export interface DailyMissionClaims {
...
export interface DailyRetentionState {
  ...
  readonly lastRewardClaimDayKey: string | null;
  readonly counters: DailyMissionCounters;
  readonly claimed: DailyMissionClaims;
}
...
export interface DailyClaimResult {
  ...
  readonly reward: DailyReward | null;
...
const DAILY_REWARDS: readonly DailyReward[] = [
  ...
  { day: 5, coins: 200, coreShards: 0 },
  { day: 6, coins: 260, coreShards: 0 },
  { day: 7, coins: 320, coreShards: 1 }
] as const;
...
export function createDefaultDailyState(now = Date.now()): DailyRetentionState {
  return {
    dayKey: utcDayKey(now),
    streak: 0,
    lastRewardClaimDayKey: null,
    counters: { merge: 0, defeat: 0, recruit: 0 },
    claimed: { merge: false, defeat: false, recruit: false }
  };
}

export function rollDailyState(state: DailyRetentionState, now = Date.now()): DailyRetentionState {
...
export function canClaimDailyReward(state: DailyRetentionState, now = Date.now()): boolean {
...
export function claimDailyReward(state: DailyRetentionState, now = Date.now()): DailyClaimResult {
  ...
  const today = utcDayKey(now);
  if (current.lastRewardClaimDayKey === today) return { claimed: false, reward: null, state: current };
  ...
  return {
    claimed: true,
    reward,
    state: { ...current, streak, lastRewardClaimDayKey: today }
  };
}

export function recordDailyAction(
  state: DailyRetentionState,
  action: DailyMissionId,
  amount = 1,
  now = Date.now()
): DailyRetentionState {
  const current = rollDailyState(state, now);
  const definition = getDailyMission(action);
  const nextValue = Math.min(definition.target, current.counters[action] + Math.max(0, Math.floor(amount)));
  return { ...current, counters: { ...current.counters, [action]: nextValue } };
}
