...
export class CollectionPanel {
  ...
  public constructor(
    ...
    private readonly onClaimAlbumMilestone: (target: number) => void
  ) {}
  ...
  public create(): void {
    ...
    const title = this.scene.add.text(-305, -657, translate('codex.title'), {
      fontFamily: 'Arial Black, system-ui, sans-serif', fontSize: '39px', color: '#f5fbff', stroke: '#151a39', strokeThickness: 8
    });
    this.titleCount = this.scene.add.text(
      -303,
      -607,
      translate('codex.discovered', { current: 0, total: getAllCreatures().length }),
    ...
    const collectionLabel = this.scene.add.text(-420, -532, translate('codex.discoveries'), {
      fontFamily: 'Arial Black, system-ui, sans-serif', fontSize: '23px', color: '#ffd984'
    });
    ...
    this.pageText = this.scene.add.text(0, -105, '', {
      fontFamily: 'Arial Black, system-ui, sans-serif', fontSize: '15px', color: '#bfefff', stroke: '#171a37', strokeThickness: 4
    }).setOrigin(0.5);

    const albumLabel = this.scene.add.text(-420, -54, translate('codex.album'), {
      fontFamily: 'Arial Black, system-ui, sans-serif', fontSize: '23px', color: '#ffd984'
    });
    this.albumRoot = this.scene.add.container(0, 0);

    const achievementLabel = this.scene.add.text(-420, 112, translate('codex.achievements'), {
      fontFamily: 'Arial Black, system-ui, sans-serif', fontSize: '23px', color: '#ffd984'
    });
    this.achievementPrevButton = this.createAchievementPageButton(292, 127, '‹', () => this.changeAchievementPage(-1));
    this.achievementPageText = this.scene.add.text(350, 127, '1 / 3', {
      fontFamily: 'Arial Black, system-ui, sans-serif', fontSize: '14px', color: '#bfefff'
    }).setOrigin(0.5);
    this.achievementNextButton = this.createAchievementPageButton(408, 127, '›', () => this.changeAchievementPage(1));
    ...
    this.root = this.scene.add.container(540, 960, [
      ...
      this.albumRoot,
      achievementLabel,
      this.achievementPrevButton,
      this.achievementPageText,
      this.achievementNextButton,
      this.cardsRoot,
  ...
  private createRarityPill(
    x: number,
    y: number,
    mutationId: Exclude<MutationId, 'none'>,
    width: number
  ...
  private createPageButton(x: number, y: number, label: string, onPress: () => void): Phaser.GameObjects.Container {
    ...
    const text = this.scene.add.text(0, -2, label, {
      fontFamily: 'Arial Black, system-ui, sans-serif', fontSize: '32px', color: '#eaffff'
    }).setOrigin(0.5);
  ...
  private createAchievementPageButton(x: number, y: number, label: string, onPress: () => void): Phaser.GameObjects.Container {
    ...
    const text = this.scene.add.text(0, -1, label, {
      fontFamily: 'Arial Black, system-ui, sans-serif', fontSize: '23px', color: '#eaffff'
    }).setOrigin(0.5);
    const button = this.scene.add.container(x, y, [bg, text]);
  ...
  private renderCreatureCards(progress: CollectionProgress, album: MutationAlbumProgress): void {
    ...
    this.pageText.setText(translate('codex.forms', {
      start: start + 1,
      end,
    ...
    visible.forEach((creature, index) => {
      ...
      const art = this.scene.add.image(0, -22, creature.texture).setDisplaySize(88, 88);
      if (!discovered) art.setTint(0x151827).setAlpha(0.42);
      const name = this.scene.add.text(0, 35, discovered ? creature.name.toUpperCase() : '???', {
        fontFamily: 'Arial Black, system-ui, sans-serif',
        fontSize: discovered ? '11px' : '13px',
        color: discovered ? '#f5fbff' : '#77809f',
        align: 'center'
      }).setOrigin(0.5).setWordWrapWidth(150);
      const tier = this.scene.add.text(66, -57, `T${creature.level}`, {
        fontFamily: 'Arial Black, system-ui, sans-serif', fontSize: '13px', color: discovered ? '#ffe69a' : '#59617e'
      }).setOrigin(0.5);

      const markers: Phaser.GameObjects.GameObject[] = [];
      if (isCollectionKey(creature.key)) {
        MUTATION_IDS.forEach((mutation, mutationIndex) => {
          markers.push(this.createVariantMarker(
            -48 + mutationIndex * 32,
            63,
            creature.key,
            mutation,
  ...
  private createVariantMarker(
    x: number,
    y: number,
    creature: CollectionKey,
    mutation: MutationId,
    normalColor: number,
    album: MutationAlbumProgress
  ): Phaser.GameObjects.Rectangle {
    const unlocked = album.discovered.includes(mutationAlbumKey(creature, mutation));
    const accent = mutation === 'none' ? normalColor : getMutationDefinition(mutation).accentColor;
    return this.scene.add.rectangle(x, y, 12, 12, unlocked ? accent : 0x343a5d, unlocked ? 1 : 0.55)
      .setStrokeStyle(2, unlocked ? 0xffffff : 0x68708f, unlocked ? 0.6 : 0.24)
      .setRotation(Math.PI / 4);
  }

  private renderAlbum(album: MutationAlbumProgress): void {
    this.albumRoot.removeAll(true);
    const completion = mutationAlbumCompletion(album);
    const next = nextMutationAlbumMilestone(album);

    const summary = this.scene.add.text(-420, -20, translate('codex.albumProgress', completion), {
      fontFamily: 'system-ui, sans-serif', fontStyle: '800', fontSize: '17px', color: '#bfefff'
    });

    const track = this.scene.add.graphics();
    track.fillStyle(0x0d1228, 0.96);
    track.fillRoundedRect(-420, 13, 500, 18, 9);
    track.fillStyle(0x72e5ff, 0.92);
    track.fillRoundedRect(-420, 13, Math.max(8, 500 * (completion.percent / 100)), 18, 9);

    const milestoneBg = this.scene.add.graphics();
    const ready = next !== null && completion.current >= next.target;
    milestoneBg.fillStyle(ready ? 0x5a4126 : 0x242b4d, 0.98);
    milestoneBg.fillRoundedRect(-148, -35, 296, 74, 24);
    milestoneBg.lineStyle(2, ready ? 0xffd768 : 0x7181ad, ready ? 0.72 : 0.28);
    milestoneBg.strokeRoundedRect(-148, -35, 296, 74, 24);

    let milestoneTitle: string;
    let rewardText = '';
    if (!next) {
      milestoneTitle = translate('codex.albumComplete');
    } else {
      milestoneTitle = translate('codex.albumNext', { current: completion.current, target: next.target });
      rewardText = next.coreShards > 0
        ? `${next.coreShards} ${translate(next.coreShards === 1 ? 'common.shard' : 'common.shards')}`
        : `${next.coins} ${translate('common.coins')}`;
    }

    const milestoneText = this.scene.add.text(0, -12, milestoneTitle, {
      fontFamily: 'Arial Black, system-ui, sans-serif', fontSize: '12px', color: ready ? '#fff3c4' : '#dce7ff', align: 'center'
    }).setOrigin(0.5);
    const rewardLabel = this.scene.add.text(0, 13, ready && next ? `${translate('common.claim')} • ${rewardText}` : rewardText, {
      fontFamily: 'Arial Black, system-ui, sans-serif', fontSize: ready ? '14px' : '13px', color: ready ? '#ffd768' : '#9eacd0', align: 'center'
    }).setOrigin(0.5);
    const milestone = this.scene.add.container(268, 0, [milestoneBg, milestoneText, rewardLabel]);

    if (ready && next) {
      milestone.setSize(304, 82).setInteractive({ useHandCursor: true });
      milestone.on('pointerdown', () => {
        this.scene.tweens.add({ targets: milestone, scaleX: 0.95, scaleY: 0.95, duration: 75, yoyo: true, ease: 'Quad.Out' });
        this.onClaimAlbumMilestone(next.target);
      });
    }

    const ascend = this.scene.add.text(-420, 49, translate('codex.ascend'), {
      fontFamily: 'Arial Black, system-ui, sans-serif', fontSize: '12px', color: '#7ea4c7'
    });

    this.albumRoot.add([summary, track, milestone, ascend]);
  }
  ...
  private renderAchievements(progress: CollectionProgress): void {
    ...
    visible.forEach((achievement, index) => {
      ...
      row.fillRoundedRect(-412, -33, 824, 66, 22);
      row.lineStyle(2, status.ready ? 0xffd768 : 0x7080ad, status.ready ? 0.58 : 0.18);
      row.strokeRoundedRect(-412, -33, 824, 66, 22);
      const title = this.scene.add.text(-384, -24, achievement.name, {
        fontFamily: 'Arial Black, system-ui, sans-serif', fontSize: '16px', color: status.claimed ? '#8fe8bb' : '#f4f7ff'
      });
      const progressText = this.scene.add.text(-384, 0, `${achievement.description}   ${status.current}/${status.target}`, {
        fontFamily: 'system-ui, sans-serif', fontStyle: '700', fontSize: '13px', color: '#aab6d8'
      });
      const rewardText = achievement.reward.coreShards > 0
        ? `${achievement.reward.coreShards} ${translate(achievement.reward.coreShards === 1 ? 'common.shard' : 'common.shards')}`
        : `${achievement.reward.coins} ${translate('common.coins')}`;
      const buttonBg = this.scene.add.graphics();
      buttonBg.fillStyle(status.claimed ? 0x31526a : status.ready ? 0xf6b94b : 0x353b5b, 1);
      buttonBg.fillRoundedRect(-96, -24, 192, 48, 20);
      const buttonText = this.scene.add.text(
        0,
        0,
        status.claimed ? translate('common.claimed') : status.ready ? `${translate('common.claim')} ${rewardText}` : rewardText,
        {
          fontFamily: 'Arial Black, system-ui, sans-serif',
          fontSize: status.ready ? '12px' : '14px',
          color: status.ready ? '#3b2718' : '#cad2e8',
          align: 'center'
        }
      ).setOrigin(0.5);
      const button = this.scene.add.container(300, 0, [buttonBg, buttonText]);
      if (status.ready) {
        button.setSize(192, 52).setInteractive({ useHandCursor: true });
        button.on('pointerdown', () => {
