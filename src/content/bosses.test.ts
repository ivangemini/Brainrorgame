...
describe('boss roster', () => {
  it('ships four original bosses with unique runtime identities', () => {
    ...
    expect(new Set(bosses.map((boss) => boss.presentation.defeatStyle)).size).toBe(bosses.length);
    expect(new Set(bosses.map((boss) => boss.presentation.idleStyle)).size).toBe(bosses.length);
  });
  ...
  it('keeps Kettlestar as a pressure-burst profile without exceeding its cap', () => {
